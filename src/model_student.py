...
class MLP_AttentionReplacement(nn.Module):
    """
    dim -> out_dim
    """
    def __init__(self, dim, out_dim, dropout=0.1):
        ...
        self.num_tokens = 22  # joints

        in_features = 22 * dim
        out_features = 22 * out_dim
    ...
    def forward(self, x):
        # x: (B, T, dim)
        B, J, D = x.shape
        assert J == self.num_tokens, f"expected T={self.num_tokens}, got {J}"
        ...
        # (B, J*out_dim) -> (B, J, out_dim)
        x = x.reshape(B, J, self.out_dim)
        return x
...
class NotTransformer(nn.Module):
    def __init__(self, dim, depth, heads, mlp_dim, dropout):
        super().__init__()
        self.layers = nn.ModuleList([])
        if dim == mlp_dim:
                ...
                self.layers.append(
                    nn.ModuleList(
                        [
                            LayerNormalize(
                                    dim,
                                    MLP_AttentionReplacement(dim, mlp_dim, dropout=dropout),
                                ),
                            Residual(
                                LayerNormalize(
                                    mlp_dim,
            ...
            for _ in range(depth):
                self.layers.append(
                    nn.ModuleList(
                        [
                            LayerNormalize(dim, MLP_AttentionReplacement(dim, mlp_dim, dropout=dropout)),
                            Residual(
                                LayerNormalize(
                                    mlp_dim,
                                    MLP_Block(mlp_dim, mlp_dim * 2, dropout=dropout),
...
class DistilledRetNet(nn.Module):
    ...
    def forward(
        ...
        if type(local_mean) == np.ndarray:
                local_mean = torch.from_numpy(local_mean)
                local_std = torch.from_numpy(local_std)
        local_mean = local_mean.float().to(seqA.device)
        local_std = local_std.float().to(seqA.device)
        ...
        # print(f"Preprocessing: {time.time() - st}")

        """ mapping from A to B frame by frame"""
        # time_per_frame = []
        # dl_per_frame = []
        # skel_time_per_frame = []
        # shape_time_per_frame = []
        # balance_time_per_frame = []
        # ik_time_per_frame = []
        for t in range(T):
            ...
            if phase=="train":
                ...
                if one_w:
                    bala_gate = torch.ones(bala_gate.shape, dtype=torch.float32).to(
                        seqA.device
                    )
            ...
            # dl_per_frame.append(start.elapsed_time(end))
        ...
        #     time_per_frame.append(time.time() - st_)
        #     skel_time_per_frame.append(st_skel - st_)
        #     shape_time_per_frame.append(st_shape - st_skel)
        #     balance_time_per_frame.append(st_bala - st_shape)
        #     ik_time_per_frame.append(time.time() - st_bala)
        # print(f"Total One Processing: {np.mean(time_per_frame)}")
        # print(f"DL One Processing: {np.mean(dl_per_frame)}")
        # print(f"Skel One Processing: {np.mean(skel_time_per_frame)}")
        # print(f"Shape One Processing: {np.mean(shape_time_per_frame)}")
        # print(f"Bala One Processing: {np.mean(balance_time_per_frame)}")
        # print(f"IK One Processing: {np.mean(ik_time_per_frame)}")
        # print(f"All Processing: {time.time() - st}")
    ...
    @staticmethod
    def get_quat_loss_(
        mask, quatB_gt, quatB_rt
    ):
        quat_loss = torch.sum(
            (
                torch.multiply(
                    mask[:, :, None, None], torch.subtract(quatB_gt, quatB_rt)
                )
            )
            ** 2,
            dim=[0, 1, 3],
        )
        quat_loss = torch.divide(
            quat_loss,
            torch.maximum(torch.sum(mask), torch.tensor(1).to(quatB_gt.device)),
        )
        return quat_loss

    @staticmethod
    def get_quat_loss(
        joint_lst, num_joint, foot_contact_mask, mask, quatB_gt, quatB_rt
    ):
        mask_ = mask.unsqueeze(2)
        not_foot_contact_mask = torch.ones_like(quatB_gt[:,:,:,0]).to(quatB_gt.device)
        not_foot_contact_mask[:,:,joint_lst] = 1 - foot_contact_mask
        

        quat_loss = torch.sum(
            (
                torch.multiply(
                    mask_[:, :, :, None] * not_foot_contact_mask[:, :, :, None], 
        ...
        quat_loss = torch.divide(
            quat_loss,
            torch.maximum(torch.sum(mask_ * not_foot_contact_mask), torch.tensor(1).to(quatB_gt.device)),
        )
        return quat_loss

    @staticmethod
    def get_foot_contact_loss(
        joint_lst, num_joint, foot_contact_mask, mask, localb_rt, localb_gt
    ):
        mask_ = mask.unsqueeze(2)

        foot_contact_loss = torch.sum(
            (
                torch.multiply(
                    mask_[:, :, :, None] * foot_contact_mask[:, :, :, None], 
                    torch.subtract(localb_gt[:,:,joint_lst], localb_rt[:,:,joint_lst])
                )
            )
            ** 2,
            dim=[0, 1, 3],
        )
        foot_contact_loss = torch.divide(
            foot_contact_loss,
            torch.maximum(torch.sum(mask_ * foot_contact_mask), torch.tensor(1).to(localb_gt.device)),
        )
    ...
    @staticmethod
    def get_foot_contact_loss_(
        joint_lst, foot_contact_mask, mask, heightB, localB_rt_denorm, globalB_rt, offsetB
    ):
        mask_ = mask.unsqueeze(2)

        global_joints_y = get_global_position_y(localB_rt_denorm, globalB_rt, joint_lst, offsetB)

        foot_contact_loss = torch.sum(
            (
                torch.multiply(
                    mask_[:, :, :] * foot_contact_mask[:, :, :], 
                    global_joints_y
                )
            )
            ** 2, 
        ...
        foot_contact_loss = torch.divide(
            foot_contact_loss,
            torch.maximum(torch.sum(mask_ * foot_contact_mask), torch.tensor(1).to(localB_rt_denorm.device)),
        )
        ...
        foot_contact_loss[[0, 2]] = 0

        foot_contact_vel_loss = torch.sum(
            (
                torch.multiply(
                    mask_[:, 1:, :, None] * foot_contact_mask[:, 1:, :, None], 
                    torch.subtract(localB_rt_denorm[:,1:,joint_lst], localB_rt_denorm[:,:-1,joint_lst])
                )
            )
            ** 2,
            dim=[0, 1, 3],
        )
        foot_contact_vel_loss = torch.divide(
            foot_contact_vel_loss,
            torch.maximum(torch.sum(mask_[:,1:] * foot_contact_mask[:,1:]), torch.tensor(1).to(localB_rt_denorm.device)),
        )
        ...
        return foot_contact_loss + foot_contact_vel_loss #, global_joints_y
    ...
    def get_recon_loss(
        ...
        attW = torch.ones(num_joint).to(localB_rt.device)
        attW[atte_lst] = 2
        ...
        local_ae_loss = torch.divide(
            local_ae_loss,
            torch.maximum(
                torch.sum(ae_reg * mask), torch.tensor(1).to(ae_reg.device)
            ),
        )
        ...
        for i in range(T):
            ...
            if ifth:
                phi_val_la = (
                    phi_val_la
                    if phi_val_la > 3.0
                    else torch.tensor(0, dtype=torch.float32, requires_grad=True).to(
                        phi_val_la.device
                    )
                )
...
def get_global_position_y(
        localB, globalB, joint_lst, offsetB
    ):
        joints_y = localB[:,:,joint_lst, 1]
        root_y = globalB[...,-3] # B T 4 -> B T
        offset_y = offsetB[..., 1] # B 3 -> B

        translation_y = torch.zeros_like(root_y)
        translation_y[:,1:] = torch.cumsum(root_y[:,:-1], dim=1)
        translation_y += offset_y[:,None]
        translation_y = translation_y[...,None]

        global_joints_y = joints_y + translation_y
        return global_joints_y
