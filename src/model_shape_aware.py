...
class RetNet(nn.Module):
    ...
    def forward(
        ...
        local_mean = torch.from_numpy(local_mean).float().to(seqA.device)
        local_std = torch.from_numpy(local_std).float().to(seqA.device)
    ...
    def get_cons_loss(
        ...
        local_ae_loss = torch.divide(
            local_ae_loss,
            torch.maximum(torch.sum(mask), torch.tensor(1).to(localB_base.device)),
        )
    ...
    def get_rot_cons_loss(alpha, euler_ord, quatB_rt):
        ...
        twistB_loss = torch.mean(
            torch.square(
                torch.maximum(
                    torch.tensor(0).to(quatB_rt.device),
    ...
    def get_gen_loss(fake_score, ae_reg):
        ...
        gen_motion_loss = bceloss(
            fake_score, torch.ones(fake_score.shape).to(fake_score.device)
        )
        gen_motion_loss = torch.sum(torch.multiply((1 - ae_reg), gen_motion_loss))
        gen_motion_loss = torch.divide(
            gen_motion_loss,
            torch.maximum(torch.sum(1 - ae_reg), torch.tensor(1).to(ae_reg.device)),
        )
    ...
    def get_rela_loss(rA, rB, mask):
        ...
        rela_loss = torch.divide(
            rela_loss, torch.maximum(torch.sum(mask), torch.tensor(1).to(mask.device))
        )
    ...
    def get_dis_loss(real_score, fake_score, ae_reg):
        bceloss = nn.BCELoss(reduction='none')
        gen_motion_loss = bceloss(
            fake_score, torch.zeros(fake_score.shape).to(fake_score.device)
        )
        gen_motion_loss = torch.sum(torch.multiply((1 - ae_reg), gen_motion_loss))
        gen_motion_loss = torch.divide(
            gen_motion_loss,
        ...
        dis_motion_loss = bceloss(
            real_score, torch.ones(real_score.shape).to(real_score.device)
        )
