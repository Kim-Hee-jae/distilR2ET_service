...
class Feeder(Dataset):
    def __init__(self, data_path, stats_path, shape_path, gt_path, max_length, data_config_path, phase): # source_chars, target_char
        ...
        self.shape_dic = {}
        shape_lst = []
        file_names = self.source_chars + [self.target_char]
    ...
    def load_data(self):
        ...
        for folder_name in folders:
            ...
            for cfile in files:
                ...
                positions_B = np.load(
                    join(self.data_path, self.target_char, file_name + "_skel.npy")
                )
                ...
                sequence_B = np.load(
                    join(self.data_path, self.target_char, file_name + "_seq.npy")
                )
                ...
                # ground truth quat (#frames, #joints, 4)
                quat = np.load(
                    join(self.gt_path, self.target_char, folder_name, file_name + ".npz")
                )
