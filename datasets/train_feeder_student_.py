...
class Feeder(Dataset):
    def __init__(self, data_path, stats_path, shape_path, target_path, max_length):
    ...
    def load_data(self):
        ...
        for folder_name in folders:
            ...
            for cfile in files:
                ...
                all_quats_A.append(quat)
                
                 # ground truth quat (#frames, #joints, 4)
                quat = np.load(
                    join(self.target_path, folder_name, file_name + ".npz")
                )
                all_quats_B.append(quat['delta_qs'][0])
        ...
        allframes_quat_A = np.concatenate(all_quats_A)
        quat_mean_A = allframes_quat_A.mean(axis=0)[None, :]  # 1 J 4
        quat_std_A = allframes_quat_A.std(axis=0)[None, :]
        
        allframes_quat_B = np.concatenate(all_quats_B)
        quat_mean_B = allframes_quat_B.mean(axis=0)[None, :]  # 1 J 4
        quat_std_B = allframes_quat_B.std(axis=0)[None, :]
        ...
        np.save(join(self.stats_path, "mixamo_quat_mean_A.npy"), quat_mean_A)
        np.save(join(self.stats_path, "mixamo_quat_std_A.npy"), quat_std_A)
        
        np.save(join(self.stats_path, "mixamo_quat_mean_B.npy"), quat_mean_B)
        np.save(join(self.stats_path, "mixamo_quat_std_B.npy"), quat_std_B)
        ...
        for i in range(len(train_local)):
            ...
            all_quats_A[i] = (all_quats_A[i] - quat_mean_A) / quat_std_A
            all_quats_B[i] = (all_quats_B[i] - quat_mean_B) / quat_std_B
        ...
        self.quat_mean_A = quat_mean_A
        self.quat_std_A = quat_std_A
        self.quat_mean_B = quat_mean_B
        self.quat_std_B = quat_std_B
        self.global_mean = global_mean
        self.global_std = global_std
        self.all_names = all_names
        self.seq_names = seq_names
        self.all_quats_A = all_quats_A
        self.all_quats_B = all_quats_B
    ...
    def __getitem__(self, indexA):
        ...
        quat_i = self.all_quats_A[indexA]
        quat_j = self.all_quats_B[indexA]
        ...
        cquatB = quat_j[stidx : (stidx + max_len)]
        if cquatB.shape[0] < max_len:
            zeros = np.zeros((max_len - cquatB.shape[0], n_joints, 4))
            zeros[:, :, 0] = 1.0
            cquatB = np.concatenate((cquatB, zeros))
        ...
        quatB = cquatB.astype(np.float32)
